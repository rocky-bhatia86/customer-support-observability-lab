...
This guide assumes nothing about the machine being used. Follow it in
...
Note: the `[dev]` extra is required. It installs `pytest`, used in Section
5. Without it, `pip install -e .` alone will not include `pytest`.
...
blank unless pointing at a specific OpenAI-compatible gateway, for example
an internal model-serving endpoint.
...
1. Click any suggested ticket. It resolves within roughly 10 to 20 seconds.
...
2. Open **Show Agent Activity** and confirm real agent names and real data
   are shown. Each specialist agent data lookup is a real MCP tool call to
   `support_ai.mcp_server`, a subprocess spawned on first use and reused for
   the life of the server.
3. Switch the header toggle to **v2** and re-run the same ticket. Some
   tickets, for example a refund request, will now loop and escalate on the
   identical code, with only one setting changed. This is the intended
   behavior, referred to as Failure D, and is not a defect.
...
where an unset `base_url` resolves incorrectly. Confirm the code includes
this fix. If not, re-copy the repository.
...
2. In a separate folder outside this repository:
   ...
   docker compose up -d
   ```
3. After 1 to 2 minutes, open **http://localhost:3000**, create a local
   account and project, and generate an API key pair.
4. Add to this repository .env file:
   ```
   LANGFUSE_PUBLIC_KEY=pk-lf-your key
   LANGFUSE_SECRET_KEY=sk-lf-your key
   ...
   **Port conflicts:** if `docker compose up -d` reports a port already in
   use, another process on the machine is occupying one of the Langfuse
   ports (commonly 5432, 6379, 8123, 9000, 9090, or 9091). Edit
   `docker-compose.yml` in the `langfuse-local` folder and remap the host
   side of the conflicting `ports:` entry, for example `5432:5432` to
   `15432:5432`. The container side must stay unchanged.
...
python scripts/run_server.py
```
Run a ticket, then open the Langfuse project (Cloud or local) and navigate
to **Tracing → Traces**. A `support_ticket_workflow` trace should appear
with a nested span tree: classify, then retrieve or lookup, then draft,
then check.
...
Each run prints a dataset run URL with per-ticket scores. In the Langfuse
...
All tests should pass, 7 in total, on `main` and later branches. A failure
immediately after moving to a new machine is almost always a missed
`pip install -e ".[dev]"` after a branch switch, or a `.venv` folder that
did not transfer correctly. Delete `.venv` and recreate it using the
...
- The defaults shipped in `.env.example` are assumed correct. Only
  `OPENAI_API_KEY`, and later the Langfuse keys, require values.
- Longer response times are expected behavior, not a fault. Real,
  multi-iteration LLM calls under `v2` can take 30 to 90 seconds. The chat
  UI live step-by-step activity indicates progress. A static screen for a
  few seconds between steps is normal.
